...
export default function Home() {
  ...
  const getRouteData = (path) => {
    ...
    if (routesData[path]) {
      ...
      data.title = `${route.heading} | Urgent Taxis`;
      data.desc = route.faqs?.[0]?.a || data.desc;
      data.h1 = <>{route.heading.replace('Taxi', '')} <br/><span className="text-[#1e3b8a]">Taxi</span></>;
      data.subtitle = `Check latest ${route.heading} fare. Book instantly.`;
      data.heading = route.heading;
    }
    return data;
  };
